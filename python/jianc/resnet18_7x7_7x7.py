...
class ResidualBlock(nn.Module):
    def __init__(self, inchannel, outchannel, stride=1):
        ...
        self.left = nn.Sequential(
            nn.Conv2d(inchannel, outchannel, kernel_size=7, stride=stride, padding=3, bias=False),
            nn.BatchNorm2d(outchannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(outchannel, outchannel, kernel_size=7, stride=1, padding=3, bias=False),
            nn.BatchNorm2d(outchannel)
        )
