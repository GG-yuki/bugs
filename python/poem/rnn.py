...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim):
        ...
        # ???
        self.rnn_lstm = torch.nn.LSTM(input_size=self.word_embedding_dim, hidden_size=self.lstm_dim, num_layers=2)
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        h0 = torch.zeros((2, batch_input.shape[1], self.lstm_dim))
        if torch.cuda.is_available():
            h0 = h0.cuda()
        output,(hn,cn) = self.rnn_lstm(batch_input, (h0,h0))
