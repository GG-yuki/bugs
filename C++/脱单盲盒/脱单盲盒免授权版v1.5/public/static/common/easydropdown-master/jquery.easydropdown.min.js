...
d(this);c.is(":selected")&&(b.selected={index:a,title:c.html()},b.focusIndex=a);c.hasClass("label")&&0==a?(b.hasLabel=!0,b.label=c.html(),c.attr("value","")):b.options.push({domNode:c[0],title:c.html(),value:c.val(),selected:c.is(":selected")})}),b.selected||(b.selected={index:0,title:b.$options.eq(0).html()},b.focusIndex=0),b.render())},render:function(){var a=this;a.$container=a.$select.wrap('<div class="'+a.wrapperClass+(a.isTouch&&a.nativeTouch?" touch":"")+(a.disabled?" disabled":"")+'"><span class="old"/></div>').parent().parent();
    a.$active = d('<span class="selected">' + a.selected.title + "</span>").appendTo(a.$container); a.$carat = d('<span class="carat"/>').appendTo(a.$container); a.$scrollWrapper = d("<div><ul/></div>").appendTo(a.$container); a.$dropDown = a.$scrollWrapper.find("ul"); a.$form = a.$container.closest("form"); d.each(a.options, function () { a.$dropDown.append("<li" + (this.selected ? ' class="active"' : "") + ">" + (this.title.indexOf("--") < 0 ? "<b>" : "") + this.title + (this.title.indexOf("--") < 0 ? "</b>" : "") + "</li>") }); a.$items = a.$dropDown.find("li"); a.maxHeight = 0; a.cutOff && a.$items.length > a.cutOff && a.$container.addClass("scrollable");
for(i=0;i<a.$items.length;i++){var c=a.$items.eq(i);a.maxHeight+=c.outerHeight();if(a.cutOff==i+1)break}a.isTouch&&a.nativeTouch?a.bindTouchHandlers():a.bindHandlers()},bindTouchHandlers:function(){var a=this;a.$container.on("click.easyDropDown",function(){a.$select.focus()});a.$select.on({change:function(){var c=d(this).find("option:selected"),b=c.html(),c=c.val();a.$active.html(b);"function"===typeof a.onChange&&a.onChange.call(a.$select[0],{title:b,value:c})},focus:function(){a.$container.addClass("focus")},
